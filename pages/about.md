---
title: About
layout: about
permalink: /about.html
# include CollectionBuilder info at bottom
credits: true
# Edit the markdown on in this file to describe your collection
# Look in _includes/feature for options to easily add features to the page
---
## About the Collection
This collection is built to support the learning objectives, outcomes, and assignments for the Creative Commons Certificate for Academic Librarians. It is built as a test case for creating a collection of materials used by my own research group and the feasibility of using CollectionBuilder to house everything. I specifically chose to use images that would be similar to those we would have in our own collection and integrate into a collection like this to explore what types of licensed are used for these archaeological materials and publications. 

### Collection License 
[Murine Molar Collection](https://aeberuw.github.io/CCCertificateCollection/) by [Anneliese Eber](https://orcid.org/0009-0000-8922-8877) is licensed under [CC BY-NC-SA 4.0](http://creativecommons.org/licenses/by-nc-sa/4.0). 

After looking at the range of CC licenses for the individual items in this collection, I made note of any specific requirements that would need to be considered in the overall collection license (e.g., NonCommercial elements). Since this collection does include items with NonCommercial license elements, I needed to make sure my collection license followed these terms and included the NonCommercial element. Using the [Collection Licenses Chart](https://creativecommons.org/faq/#if-i-create-a-collection-that-includes-a-work-offered-under-a-cc-license-which-licenses-may-i-choose-for-the-collection), I confirmed which licenses were available to me to use and selected [CC BY-NC-SA 4.0](http://creativecommons.org/licenses/by-nc-sa/4.0). 

#### Creating a Remix Instead of a Collection

If I were to take these materials and remix them instead, I would have to remove some of the materials in them to accommodate the NoDerivatives element on one of the items. Without removing it, I would have no licensing options for the collection as creating the remix would violate the terms of the CC BY-NC-ND license. Once this item was removed, I would still be limited to licenses including a NonCommercial element, and my license options would not change. 

### References
- “2.1 Copyright Basics” (<https://creativecommons.org/course/cc-cert-edu/unit-2-copyright-law/2-1-copyright-basics/>) by Creative Commons. CC BY 4.0.
- “4.1 Choosing and Applying a CC License” (<https://creativecommons.org/course/cc-cert-edu/unit-4-using-cc-licenses-and-cc-licensed-works/4-1-choosing-and-applying-a-cc-license/>) by Creative Commons. CC BY 4.0.
- “4.2 Things to Consider after CC Licensing” (<https://creativecommons.org/course/cc-cert-edu/unit-4-using-cc-licenses-and-cc-licensed-works/4-2-things-to-consider-after-cc-licensing/>) by Creative Commons. CC BY 4.0.
- “4.3 Finding and Reusing CC-Licensed Work” (<https://creativecommons.org/course/cc-cert-edu/unit-4-using-cc-licenses-and-cc-licensed-works/4-3-finding-and-reusing-cc-licensed-work/>) by Creative Commons. CC BY 4.0.
- “4.4 Remixing CC-Licensed Work” (<https://creativecommons.org/course/cc-cert-edu/unit-4-using-cc-licenses-and-cc-licensed-works/4-4-remixing-cc-licensed-work/>) by Creative Commons. CC BY 4.0.
- Copyright Act, RSC 1985, c C-42. (<https://laws-lois.justice.gc.ca/eng/acts/C-42/page-1.html#docCont>) by the Government of Canada.
- "'Derivative Works' in Canada?" (<https://iplaw.allard.ubc.ca/2022/11/19/derivative-works-in-canada/>) by Xue Zhang. CC BY 4.0.
- "Frequently Asked Questions" (<https://creativecommons.org/faq/>) by Creative Commons. CC BY 4.0.
- "Photo" (<https://commons.wikimedia.org/wiki/File:Papagomys.jpg#file>) by Eelco Kruideniers is licensed under [CC BY 3.0](http://creativecommons.org/licenses/by/3.0)
- "Photo" (<https://commons.wikimedia.org/wiki/File:Megalomys_desmarestii_sin_i_26124_m.png>) by [Ucucha](https://commons.wikimedia.org/wiki/User:Ucucha) is licensed under [CC BY-SA 3.0](http://creativecommons.org/licenses/by-sa/3.0).
- Veatch, E. G., Tocheri, M. W., Sutikna, T., McGrath, K., Wahyu Saptomo, E., Jatmiko, & Helgen, K. M. (2019). Temporal shifts in the distribution of murine rodent body size classes at Liang Bua (Flores, Indonesia) reveal new insights into the paleoecology of Homo floresiensis and associated fauna. Journal of Human Evolution, 130, 45–60. (<https://doi.org/10.1016/j.jhevol.2019.02.002>) is licensed under [CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/). 
- Veatch, E. G., Fabre, P.-H., Tocheri, M. W., Sutikna, T., Saptomo, E. W., Musser, G. G., & Helgen, K. M. (2023). A new giant shrew rat (Rodentia, Muridae, Murinae) from Flores, Indonesia and a comparative investigation of its ecomorphology. Records of the Australian Museum, 75(5), 741–764. (<https://doi.org/10.3853/j.2201-4349.75.2023.1781>) is licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).

#### Collection Template 
This site is generated using [CollectionBuilder-GH](https://collectionbuilding.github.io/gh/)
